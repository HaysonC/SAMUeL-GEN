...
class ResNetBlock(nn.Module):
    ...
    def __init__(self, channels, cond_dim):
        ...
        self.act = nn.SiLU()
        # dilated convs to increase temporal receptive field
        self.conv1 = nn.Conv1d(channels, channels, kernel_size=3, padding=2, dilation=2)
...
class SoftAlignUNet(nn.Module):
    ...
    def __init__(self, base_ch=64, cond_dim=64, timesteps=1000):
        super().__init__()
        # down blocks for input (increased depth to 4 levels)
        self.downs = nn.ModuleList([
            ResNetBlock(base_ch * (2**i), cond_dim) for i in range(4)
        ])
        # parallel down blocks for conditioning context
        self.cond_downs = nn.ModuleList([
            Conv1dBlock(base_ch * (2**i), base_ch * (2**(i+1))) for i in range(4)
        ])
        # upsample blocks: invert downsampling on h (1024->512->256->128->64)
        self.ups = nn.ModuleList([
            nn.ConvTranspose1d(base_ch * 16, base_ch * 8,  kernel_size=4, stride=2, padding=1),   # 1024->512
            nn.ConvTranspose1d(base_ch * 16, base_ch * 4,  kernel_size=4, stride=2, padding=1),   # 1024->256
        ...
        self.cond_ups = nn.ModuleList([
            nn.ConvTranspose1d(base_ch * 16, base_ch * 16, kernel_size=4, stride=2, padding=1), # 1024->1024
            nn.ConvTranspose1d(base_ch * 16, base_ch * 8,  kernel_size=4, stride=2, padding=1), # 1024->512
        ...
        # ResNet blocks after concatenation: use cond_dim matching time_emb size
        self.up_resnets = nn.ModuleList([
            ResNetBlock(base_ch * 16, cond_dim),  # 512+512=1024
            ResNetBlock(base_ch * 8,  cond_dim),  # 256+256=512
            ResNetBlock(base_ch * 4,  cond_dim),  # 128+128=256
            ResNetBlock(base_ch * 2,  cond_dim),  # 64+64=128
        ])
        # bottleneck cross-attention between x and downsampled cond (increased to match deeper channels)
        self.mid_attn = BottleNeckCrossAttention(base_ch * 16)
        self.mid_resnet = ResNetBlock(base_ch * 16, cond_dim)
